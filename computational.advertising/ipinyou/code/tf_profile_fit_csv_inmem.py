...
def load_data(data_dir):
    ...
    df = pd.read_csv(files[0],names=headers)
    for i in range(1,len(files)):
        temp_df = pd.read_csv(files[i],names=headers)
...
#    ds = tf.data.Dataset.from_tensor_slices((x, y)).shuffle(len(x)).batch(args.batchsize)
...
## Train the model
def train_model(model, model_dir, train_x, train_y, valid_x, valid_y, batch_size, epochs, learning_rate):
    ...
    history = model.fit(x=train_x,y=train_y, batch_size=batch_size,
                        ...
                        validation_data=(valid_x,valid_y), validation_freq=1,
...
def evaluate_model(model, valid_x, valid_y):
    ...
    test_loss, test_accuracy = model.evaluate(x=valid_x, y=valid_y,
...
if __name__ == '__main__':
    ...
    # load data
    train_x,train_y = load_data(args.train_dir)
    valid_x,valid_y = load_data(args.val_dir)
    ...
    # train model            
    train_model(model, args.model_dir, train_x, train_y, valid_x, valid_y, args.batchsize, args.epochs, args.lr)  
